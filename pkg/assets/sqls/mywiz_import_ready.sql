-- IMPORT-READY COPY generated by ChatGPT
-- Notes:
-- 1) This file still expects ref_provinces, ref_cities, and ref_barangays to already exist,
--    because this uploaded SQL references them as foreign keys but does not define them.
-- 2) Fixed the leave_entitlements reset block so it no longer references `remaining_days`,
--    which is not declared in leave_entitlements in the uploaded schema.
...
-- NOTE: The original script referenced a non-existent column `remaining_days`.
-- This import-ready version removes that assignment so the SQL can import cleanly.
