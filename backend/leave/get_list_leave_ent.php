	...
	if ($result = $conn->query($sql)) {
		if ($result->num_rows > 0) {
			while ($row = $result->fetch_assoc()) {
				...
				$table .= "<tr><td class='text-center align-middle'>{$n}</td><td class='align-middle' data-column='Employee Name: '>{$full_name}</td><td class='text-center align-middle' data-column='Action: '><div class='btn btn-outline-info btn-sm btn-edit' data-id='{$employee_id}'><span class='feather icon-edit'></span></div></td></tr>";
				$n++;
			}
			$result->free();
		}
