	...
	if ($result = $conn->query($sql)) {
		if ($result->num_rows > 0) {
			while ($row = $result->fetch_assoc()) {
				...
				$table .= "<tr><td class='text-center align-middle'>{$n}</td><td class='align-middle' data-column='Role: '>{$role_name}</td><td class='align-middle' data-column='Department: '>{$dept_name}</td><td class='align-middle' data-column='Access: '>{$access}</td><td class='align-middle' data-column='Permissions: '>{$perms}</td><td class='align-middle' data-column='Description: '>{$role_desc}</td><td class='text-center align-middle' data-column='Action: '><div class='btn btn-outline-info btn-sm btn-edit' data-id='{$role_id}'><span class='feather icon-edit'></span></div><div class='btn btn-outline-danger btn-sm btn-del' data-id='{$role_id}'><span class='feather icon-trash-2'></span></div></td></tr>";
				$n++;
			}
			$result->free();
		}
