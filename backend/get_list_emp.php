...
if (isset($_GET['security']) && $_GET['security'] == '123465') {
	...
	$sql = "SELECT e.employee_id, CONCAT( e.first_name, ' ', IF(e.middle_name IS NOT NULL AND e.middle_name != '', CONCAT(LEFT(e.middle_name,1), '. '), '' ), e.surname, IF(e.suffix IS NOT NULL AND e.suffix != '', CONCAT(' ', e.suffix), '' ) ) AS full_name, e.is_active, e.employee_code, d.department_name, r.role_name FROM mgmt_employees e LEFT JOIN mgmt_departments d ON d.department_id = e.department_id LEFT JOIN mgmt_roles r ON r.role_id = e.role_id ORDER BY e.surname ASC, e.first_name ASC";
	$n = 1;
	$table = "";
