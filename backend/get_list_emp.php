	...
	if ($result = $conn->query($sql)) {
		if ($result->num_rows > 0) {
			while ($row = $result->fetch_assoc()) {
				...
				$table .= "<tr class='clickable-row' data-href='{$profilelink}' data-name='{$full_name}'><td class='text-center'>{$n}</td><td data-column='ID: '>{$emp_number}</td><td data-column='Name: '>{$full_name}</td><td data-column='Code: '>{$employee_code}</td><td data-column='Department: '>{$dept_name}</td><td data-column='Role: '>{$role_name}</td><td class='text-center' data-column='Status: '>{$status_badge}</td><td class='text-center' data-column='Action: '><div class='btn btn-outline-info btn-sm btn-edit' data-id='{$employee_id}'><span class='feather icon-edit'></span></div><div class='btn btn-outline-warning btn-sm btn-rst' data-id='{$employee_id}'><span class='feather icon-refresh-cw'></span></div></td></tr>";	
