...
if (isset($_GET['security']) && $_GET['security'] == '123465' && isset($_GET['id'])) {
	...
	// Get the role main details
	$sql = "SELECT role_name, role_description, department_id, access_level
			FROM mgmt_roles
			WHERE role_id = '$id'";
	...
	if ($result = $conn->query($sql)) {
		if ($result->num_rows > 0) {
			...
			$role_access = $row['access_level'];

			$result->free();

			// Get role permissions (IDs)
			$role_perms = array();
			$sql2 = "SELECT permission_id
					FROM mgmt_role_permissions
					WHERE role_id = '$id'";
