...
if (isset($_SESSION['login']) && !empty($_SESSION['login'])) {
	...
	$logged_in_employee_id = (int)$_SESSION['login'];
	$is_owner = true;

	/* If profile username is passed in URL, load that user instead */
	if (isset($_GET['user']) && trim($_GET['user']) != '') {
		$username = trim($_GET['user']);
		$username_safe = $conn->real_escape_string($username);

		$sql = "SELECT  
					u.user_id, u.username, u.profile_photo, u.last_login_at,
					e.employee_id, e.employee_code, e.first_name, e.middle_name, e.surname, e.suffix,
					e.prov_id, e.city_id, e.brgy_id, e.address_line,
					e.email, e.contact_no, e.branch_id, e.department_id, e.role_id, e.is_active,
				...
				FROM mgmt_users u
				JOIN mgmt_employees e ON e.employee_id = u.employee_id
				LEFT JOIN mgmt_branch b ON b.branch_id = e.branch_id
				LEFT JOIN mgmt_departments d ON d.department_id = e.department_id
				LEFT JOIN mgmt_roles r ON r.role_id = e.role_id
		...
		$sql = "SELECT  
				...
				LEFT JOIN mgmt_departments d ON d.department_id = e.department_id
				LEFT JOIN mgmt_roles r ON r.role_id = e.role_id
				WHERE u.employee_id = '$logged_in_employee_id'
				LIMIT 1";
	}
	...
	if ($result && $result->num_rows > 0) {
		...
		/* Compare owner vs visitor */
		if ((int)$row['employee_id'] !== $logged_in_employee_id) {
			$is_owner = false;
		}
